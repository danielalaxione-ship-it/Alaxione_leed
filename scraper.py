...
def main():
    ...
    with sync_playwright() as p:
        ...
        for i, url in enumerate(place_urls):
            ...
            try:
                page.goto(url)
                # Optimization: Wait for main element dynamically instead of static 3s sleep
                try:
                    page.wait_for_selector('h1.DUwDvf', timeout=5000)
                except:
                    pass

                # Optimization: Use text_content() instead of inner_text() to avoid implicit visibility checks
