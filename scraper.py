...
def main():
    ...
    if not specialty:
        specialty = input("Enter medical specialty (e.g., 'ophtalmologue'): ")
    if not location:
        location = input("Enter location (e.g., 'Marseille'): ")
    ...
    with sync_playwright() as p:
        ...
        # Try to accept cookies if the prompt appears
        try:
        ...
        # Scroll the feed a few times to get more results
        ...
        # Find all result links and extract their URLs first to avoid DOM changes when clicking
        ...
        # Now navigate to each place URL individually to scrape its details
        for i, url in enumerate(place_urls):
            print(f"Scraping place {i+1}/{len(place_urls)}...")
            try:
                page.goto(url)
                page.wait_for_timeout(3000) # Wait for the side panel to load
                ...
                reviews = "0"
                try:
                    # Look for the 5-star rating div and its children
                ...
                if actual_website_url != "N/A":
                    print(f"Scraping email from {actual_website_url}...")
                    email = extract_email_from_url(actual_website_url)

                print(f"Extracted: {name} | Rating: {rating} | Reviews: {reviews} | Phone: {phone} | Web: {actual_website_url} | Email: {email}")
    ...
    def rating_key(lead):
        try:
            ...
            return -1.0 # Default low value for N/A or unparseable ratings
