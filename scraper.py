    ...
    with sync_playwright() as p:
        ...
        for i, url in enumerate(place_urls):
            ...
            try:
                ...
                # ⚡ Bolt Optimization: Replaced static 3s sleep with dynamic wait on the main element.
                try:
                    page.wait_for_selector('h1.DUwDvf', state='attached', timeout=5000)
                except:
                    pass

                # ⚡ Bolt Optimization: Using text_content().strip() instead of inner_text() to skip visibility checks and reduce layout thrashing overhead.
                name_locator = page.locator('h1.DUwDvf')
                name = name_locator.first.text_content().strip() if name_locator.count() > 0 else "N/A"
                ...
                phone = phone_locator.first.text_content().strip() if phone_locator.count() > 0 else "N/A"
